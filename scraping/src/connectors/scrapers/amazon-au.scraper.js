...
class AmazonAUScraper extends BaseScraper {
  constructor() {
    super('amazon-au', { rateLimitSource: 'AMAZON_AU' });
    this.baseUrl = 'https://www.amazon.com.au';
  }
  ...
  async searchProducts(keyword, maxPages = 2) {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      this.log.info(`Amazon AU: Searching "${keyword}" page ${pageNum}`);
      try {
        ...
        $('[data-component-type="s-search-result"]').each((i, el) => {
          ...
          products.push({
            asin, title, price, currency: 'AUD', rating,
            review_count: reviewCount, image_url: imageUrl,
            url: productUrl, brand, material, size,
            platform: 'amazon_au', _source: 'amazon_au',
            _keyword: keyword, _page: pageNum,
          });
        });
        allProducts.push(...products);
        this.log.info(`Amazon AU page ${pageNum}: ${products.length} products`, { keyword });
      } catch (err) {
        this.log.error(`Amazon AU page ${pageNum} failed: ${err.message}`);
      }
    }
    return allProducts;
  }
...
export const amazonAUScraper = new AmazonAUScraper();
export default amazonAUScraper;
