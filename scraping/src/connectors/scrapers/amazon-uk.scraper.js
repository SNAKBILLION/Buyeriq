// ─────────────────────────────────────────────
// BuyerIQ — Amazon UK Scraper (Cheerio + Web Unlocker)
...
class AmazonUKScraper extends BaseScraper {
  constructor() {
    super('amazon-uk', { rateLimitSource: 'AMAZON_UK' });
  ...
  async searchProducts(keyword, maxPages = 2) {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      this.log.info(`Amazon UK: Searching "${keyword}" page ${pageNum}`);
      try {
        ...
        $('[data-component-type="s-search-result"]').each((i, el) => {
          ...
          products.push({
            asin, title, price, currency: 'GBP', rating,
            review_count: reviewCount, image_url: imageUrl,
            url: productUrl, brand, material, size,
            is_prime: isPrime, platform: 'amazon_uk',
            _source: 'amazon_uk', _keyword: keyword, _page: pageNum,
          });
        });
        allProducts.push(...products);
        this.log.info(`Amazon UK page ${pageNum}: ${products.length} products`, { keyword });
      } catch (err) {
        this.log.error(`Amazon UK page ${pageNum} failed: ${err.message}`);
      }
    }
    return allProducts;
  }
...
export const amazonUKScraper = new AmazonUKScraper();
export default amazonUKScraper;
