...
function detectMaterial(title) {
  ...
  if (t.includes('bambou') || t.includes('bamboo')) return 'bamboo';
  if (t.includes('acacia')) return 'acacia';
  if (t.includes('teck') || t.includes('teak')) return 'teak';
  if (t.includes('noyer') || t.includes('walnut')) return 'walnut';
  if (t.includes('manguier') || t.includes('mango')) return 'mango';
  if (t.includes('bois') || t.includes('wood') || t.includes('wooden')) return 'wood';
  return null;
}
...
function extractSize(title) {
  ...
  const setMatch = title.match(/lot de (\d+)|set of (\d+)|(\d+)[- ]?pack/i);
  if (setMatch) return `Set of ${setMatch[1] || setMatch[2] || setMatch[3]}`;
  return null;
}

class AmazonFRScraper extends BaseScraper {
  constructor() {
    super('amazon-fr', { rateLimitSource: 'AMAZON_FR' });
    this.baseUrl = 'https://www.amazon.fr';
  }
  ...
  async searchProducts(keyword, maxPages = 2) {
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      this.log.info(`Amazon FR: Searching "${keyword}" page ${pageNum}`);
      try {
        ...
        $('[data-component-type="s-search-result"]').each((i, el) => {
          ...
          const reviewLabel = $(el).find('[aria-label*="évaluation"], [aria-label*="ratings"]').first().attr('aria-label') || '';
          ...
          products.push({
            ...
            platform: 'amazon_fr', _source: 'amazon_fr',
            _keyword: keyword, _page: pageNum,
          });
        });
        allProducts.push(...products);
        this.log.info(`Amazon FR page ${pageNum}: ${products.length} products`, { keyword });
      } catch (err) {
        this.log.error(`Amazon FR page ${pageNum} failed: ${err.message}`);
      }
    }
    return allProducts;
  }
...
export const amazonFRScraper = new AmazonFRScraper();
export default amazonFRScraper;
