...
export const getOne = asyncHandler(async (req, res) => {
  const record = await service.getByIdentifier(req.params.code);
...
export const update = asyncHandler(async (req, res) => {
  const record = await service.update(req.params.code, req.body);
  if (!record) throw (await import("../../utils/ApiError.js")).ApiError.notFound();
  res.json({ success: true, data: record });
});

export const remove = asyncHandler(async (req, res) => {
  await service.delete(req.params.code);
